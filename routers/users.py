...
def _is_email_in_use(cursor: sqlite3.Cursor, mail: str, exclude_id: int | None = None) -> bool:
    """
    [R-mail] Verilen mail kullanımda mı?
    Kullanımda sayılır: (a) aktif bir kullanıcıda, VEYA (b) silinmiş ama saklama
    süresi (90 gün) henüz dolmamış bir kayıtta. 'exclude_id' güncellemede kendi
    ...
    params: list = [mail, f"-{RETENTION_DAYS} days"]
...
def create_user(payload: UserCreate):
    ...
    try:
        ...
        if _is_email_in_use(cursor, payload.mail):
            raise HTTPException(
        ...
        if student is None:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=(
                    f"Kayıt tamamlanamadı: sistemde aktif '{DEFAULT_ROLE}' rolü "
                    f"tanımlı değil. Önce POST /roles ile '{DEFAULT_ROLE}' rolü oluşturun."
                ),
            )
        ...
        cursor.execute(
            ...
            "VALUES (?, ?, ?, ?, ?)",
            (payload.full_name, payload.mail, pwd_hash, payload.phone, payload.birth_date),
        )
...
def update_user(user_id: int, payload: UserUpdate):
    ...
    try:
        ...
        if _is_email_in_use(cursor, payload.mail, exclude_id=user_id):
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Bu e-posta kullanımda.",
            )
...
def delete_own_account(user_id: int, payload: AccountDeleteRequest):
    conn = get_connection()
    try:
        cursor = conn.cursor()
        row = cursor.execute("SELECT * FROM users WHERE id = ?", (user_id,)).fetchone()
...
def cleanup_expired(dry_run: bool = True):
    conn = get_connection()
    try:
        cursor = conn.cursor()
